// node cricinfoext.js --excel=Worldcup.csv --dataDir=worldcup --source=https://www.espncricinfo.com/series/icc-cricket-world-cup-2019-1144415/match-results
...
for(let i=0;i<matches.length;i++){
    addTeamsToTeamArrayIfMissing(teams,matches[i].t1);
    addTeamsToTeamArrayIfMissing(teams,matches[i].t2);
}

for(let i=0;i<matches.length;i++){
    addMatchToSpecificTeam(teams,matches[i].t1,matches[i].t2,
        matches[i].t1s,matches[i].t2s,matches[i].result);
    addMatchToSpecificTeam(teams,matches[i].t2,matches[i].t1,
...
//yeh function ki ladai kitni teams toatal hai iski hai
function addTeamsToTeamArrayIfMissing(teams, teamName){
    let tidx=-1;
...
//iss function ki ladai har ek teams ka kisse match hua kon jeeta scores and results ki hai
